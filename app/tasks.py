...
# def process_server(server_id: int):
#     """
#     Process a single VPNServer row: connect to its OpenVPN management port,
#     parse connected users, and sync session records.
#     """
#     db = get_db_session()
#     try:
...
#         print(f"\n{'='*60}")
#         print(f"🔍 PROCESSING: {server.name} ({ip_address}:{management_port})")
#         print(f"{'='*60}\n")
...
#             # Clear all openvpn sessions for this server when it goes down
#             db.query(VPNUserSession).filter(
#                 VPNUserSession.server_id == server_id,
#                 VPNUserSession.protocol  == 'openvpn',
#             ).delete(synchronize_session=False)
#             db.commit()
#             print(f"❌ {ip_address}:{management_port} is DOWN.")
#             return
...
#         # Phase 1: Parse CLIENT LIST for bandwidth
#         client_bandwidth = {}
...
#         # Phase 3: Build active_users map and sync sessions
#         active_users = {}
#         matched_count = 0
#         skipped_count = 0
...
#         print(f"✅ {server.name}: {len(active_users)} users (matched: {matched_count}, skipped: {skipped_count})")
...
#     Each VPNServer row is one physical server — no grouping by IP needed.
#     """
#     try:
...
#             # Deduplicate by ip:management_port so we don't hit the same machine twice
#             # (can happen if multiple app rows share an IP — but with fresh schema each
#             # physical machine+app is one row, so this is mostly a safety guard)
#             seen_ips: set = set()
...
#                     continue
#                 seen_ips.add(key)
#                 print(f"🔄 Processing {key} — {server.name}")
...
#users sessions distribution as per django logic
